...
pub fn render<'b, 's>(
    ...
    let username = state.ui.register_screen.username_input_value.to_owned();
    let password = state.ui.register_screen.password_input_value.to_owned();
                ...
                input::<'b>(bump)
                    .attr(
                        ...
                        bumpalo::collections::String::from_str_in(
                            state.ui.register_screen.username_input_value.as_str(),
                            bump,
                        )
                        .into_bump_str(),
                    )
                    .on("change", move |_root, _vdom, event| {
                        ...
                        username_change_bus.publish(Msg::Ui(ui::Msg::Register(
                            ui::register::Msg::UsernameInputChanged(text_input.value()),
                        )));
                    })
                    .finish(),
                input::<'b>(bump)
                    .attr(
                        "value",
                        bumpalo::collections::String::from_str_in(
                            state.ui.register_screen.password_input_value.as_str(),
                            bump,
                        )
                        .into_bump_str(),
                    )
                    ...
                    .on("change", move |_root, _vdom, event| {
                        ...
                        password_change_bus.publish(Msg::Ui(ui::Msg::Register(
                            ui::register::Msg::PasswordInputChanged(text_input.value()),
                        )));
                    })
                    .finish(),
                button::<'b>(bump)
                    .child(text(
                        dodrio::bumpalo::collections::String::from_str_in("Go", bump)
                            .into_bump_str(),
                    ))
                    .on("click", move |_root, _vdom, event| {
                        submission_bus.publish(Msg::Authentication(
                            authentication::Msg::RegisterRequest(RegisterRequestPayload {
